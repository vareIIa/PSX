# Handoff: abertura da estrada, etapas 3 a 7 das críticas do trailer
...
# Continuação: abertura da estrada (etapas 3 a 7 das críticas do trailer)
...
1. O personagem dirige lendo o celular e manda uma mensagem.
2. O padre aparece no farol, o carro desvia e bate numa árvore.
3. O celular vai ao chão e ele pega o aparelho.
4. Chegam mensagens "?", o padre aparece na janela e a tela fica branca.

Eu (o usuário) vi o trailer e fiz uma lista de críticas. Ela virou 7 etapas; **as
etapas 1 e 2 já estão prontas**. Sua tarefa é fazer as etapas 3 a 7, uma por vez,
...
- principalmente as memórias `etapas-do-trailer-da-estrada.md`,
  ...
  `sessoes-paralelas-no-repo.md` e `duas-armadilhas-mudas-do-godot.md`;
...
- Os arquivos são LF.
  - Faça patches com `python - <<'EOF' ... EOF` (heredoc **sem acentos**) ou com
    as ferramentas Edit/Write.
...
- **`game/src/world/motorista_cena.gd`**: as mãos e o celular do personagem.
  ...
  - Celular na mão: `_segurar_o_celular`, polegar com `teclar(uv)`,
    `repousar_em(uv)`, sinais `tecla_encostou` e `tecla_soltou`.
  ...
  - Os braços vivos são `BracoVivo` (`game/src/render/braco_vivo.gd`, shader de
    pele com unha).
- **Carro** (recém-terminado pela outra sessão; leia o estado atual antes de
  mexer):
  - `game/src/world/carro_cabine.gd`: interior. O volante é `VolanteEsportivo`
    ...
    `limpadores()`.
  - `game/src/world/carro.gd`: física.
...
- **Celular:** `game/src/world/iphone_4s.gd` (aparelho 3D),
  ...
  (a conversa: `enviar`, `receber`, `sem_teclado`, `zoom`).
...
2. **Mensagem enviada com o celular na mão.**
   - O polegar toca em `MotoristaCena.ENVIAR_NA_MAO_UV` e
     `_mandar_a_mensagem(app)` sobe o balão com "Entregue".
   - O polegar desce para `REPOUSO_DEPOIS_UV` e só então o padre é plantado no
     farol.
   - Depois da batida, o alcance ao banco ficou sem o toque. `tocar_tela()` e
     `ENVIAR_UV` (do banco) ficaram sem uso: a etapa 4 decide se apaga.

## 7. Etapas que faltam (na ordem)

### Etapa 3: batida com estrago de verdade
...
### Etapa 4: celular caindo durante a batida
- O celular deve ir ao chão NA batida, com uma queda natural e cinematográfica
  vista pelo jogador (de preferência em física ou com curva crível: quica no
  tapete e gira). Ele não deve pousar no banco e escorregar depois.
- Ajuste o trecho seguinte (hoje: vibra no banco, `alcancar_celular`,
  `derrubar_celular`, `seguir_ao_chao`) para ele já procurar o aparelho no chão.
- Pronto quando: o mosaico da batida até pegar o celular não mostra teleporte, mão
  atravessando objeto, aparelho flutuando nem braço tapando a lente.

### Etapa 5: gasolina, fumaça e fogo
...
### Etapa 6: final com o padre quebrando o vidro
- Depois do trabalho no vidro, no último instante antes do corte, o padre bate com
  as DUAS mãos no vidro e o quebra.
- A gente sente ele pegando o personagem com toda a força: mãos agarrando, tranco
  na câmera.
- No instante em que o personagem começa a ser puxado para a frente, corta para a
  tela branca (`_branco`).
- Hoje o fim é o sorriso, um bote do corpo, "tapa no vidro" e o branco (depois da
  marca "janela").
...
- **Meça antes de mexer:** registre o tempo por quadro e os picos por momento da
  ...
  - Suspeitas comuns: shader compilando na primeira vez que um efeito aparece
    (pré-aquecer), partículas e fumaça, o SubViewport da tela do celular, a malha
    dos braços refeita a cada quadro, luzes e sombras novas.
