## O braco que segura o celular do jogo: copia do `BracoVivo` da abertura, com a
## mao da `MaoDoCelular`. Copia pelo mesmo motivo do `IphoneDeJogo`: a abertura
## e trabalhada por outra frente, e a pegada do jogo foi medida contra ESTA mao —
## se a de la mudar a pose de um dedo, os dedos daqui nao podem atravessar o
## aparelho por tabela.
##
...
class_name BracoDoCelular
extends MeshInstance3D

const PELE_SHADER := """
shader_type spatial;
// Sem nevoa: o braco fica a um palmo da lente, e a neblina densa da cidade o
// pintava de branco (o aparelho ja e `fog_disabled` pelo mesmo motivo).
...
static func criar(nome: String, e_direita: bool, pele: Color, manga: Color,
		longa: bool) -> BracoDoCelular:
	var b := BracoDoCelular.new()
	b.name = nome
	b.direita = e_direita
	b._pele = pele
	b._manga = manga
...
## `d`, o dorso para `dorso`, na pose dada (nome de `MaoDoCelular.POSES` ou a
## propria pose).
static func pega(o: Vector3, d: Vector3, dorso: Vector3, pose_: Variant) -> Dictionary:
	var p: Dictionary = MaoDoCelular.pose(pose_) if pose_ is StringName or pose_ is String \
		else pose_
	var dd := d.normalized()
...
func passo(delta: float) -> void:
	...
	if _k < 1.0 and not _para.is_empty():
		...
		if _abre > 0.0:
			pegada["pose"] = MaoDoCelular.misturar(pegada["pose"], MaoDoCelular.pose(&"aberta"),
				_abre * meio)
...
func refazer() -> void:
	...
	var p := MaoDoCelular.viva(pegada["pose"], _t,
		(DEDOS_VIVOS + DEDOS_COM_MEDO * clampf(tremor, 0.0, 1.5)) * dedos_vivos, _semente)
	var punho := MaoDoCelular.punho_de(o, d, dorso)
	var cotovelo := cotovelo_entre(punho, ombro, polo)
	var dados := PSXMesh.dados_vazios()
	var b := MaoDoCelular.montar(dados, o, d, dorso, p, direita, cotovelo, _pele, _manga,
		_longa)
...
static func _misturar(a: Dictionary, b: Dictionary, k: float) -> Dictionary:
	...
	return {"o": (a["o"] as Vector3).lerp(b["o"], k), "d": bq.z, "dorso": bq.y,
		"pose": MaoDoCelular.misturar(a["pose"], b["pose"], k)}
