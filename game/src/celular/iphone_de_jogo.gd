## O iPhone do jogo: o MESMO aparelho modelado da abertura (`Iphone4S`), clonado
## para a gameplay. E o que sobe na mao do personagem quando o jogador aperta o
## celular, com o sistema (`Celular`) aceso no vidro.
##
## Por que e uma copia, e nao o `Iphone4S`
## ---------------------------------------
## A abertura e trabalhada por outra frente ao mesmo tempo, e o aparelho dela
## muda com a cena (luz, pegada, o que cai no assoalho). Aqui ele tem de ficar
## parado no que o jogo precisa: um ajuste de enquadramento da cena da estrada
## nao pode mexer no celular que o jogador usa a cada minuto, e vice-versa. A
## geometria, os shaders e as medidas sao os de la, identicos: e o mesmo iPhone.
##
## O texto abaixo e o da abertura, que continua valendo para o aparelho.
##
...
## No jogo a mao fica a um palmo e meio da tela e a rua tem luz propria: com o
## ganho da cabine escura da abertura (2,5) a pele estourava em branco.
const LUZ_MAO_GANHO := 0.9
...
var luz: OmniLight3D
## A mesma luz, so para os bracos de primeira pessoa (`BracoDoCelular.CAMADA`), com
...
## Quanto da luz da tela chega no que esta em volta. Dentro da cabine ela
## batia nos raios cromados do volante a um palmo e estourava em branco.
var luz_ganho: float = 1.0
...
func brilho(k: float) -> void:
	...
	luz.light_energy = LUZ_FORCA * k * luz_ganho
	_brilho = k
	abafar_mao(_abafa)
...
func _montar() -> void:
	...
	luz.light_cull_mask = 0xFFFFF & ~CAMADA & ~BracoDoCelular.CAMADA
	add_child(luz)
	...
	luz_da_mao.light_cull_mask = BracoDoCelular.CAMADA
