...
##
##   UV    posicao normalizada dentro da propria placa (0..1)
##   UV2   posicao em METROS no plano do vidro: `u` atravessa, `v` desce
##   COLOR r = tem limpador, g = tipo (0 para-brisa, 0,5 lateral, 1 vigia)
##
...
## Monta a malha de vidro a partir das aberturas da lataria.
##
## Devolve `{"malha": ArrayMesh, "paineis": Array}`, ou vazio se nao houver
## abertura nenhuma. Cada painel traz o proprio referencial — origem, eixo `u`
## (atravessa), eixo `v` (morro abaixo), normal e tamanho em metros — que e o que
## o limpador precisa para girar NO PLANO do vidro.
static func montar(aberturas: Array) -> Dictionary:
	var st := SurfaceTool.new()
	st.begin(Mesh.PRIMITIVE_TRIANGLES)
	var paineis: Array[Dictionary] = []
	for a: Dictionary in aberturas:
		if not TIPOS.has(a["tipo"]):
			continue
		var painel := _placa(st, a)
		if not painel.is_empty():
	...
	return {"malha": st.commit(), "paineis": paineis}


## Uma placa de vidro. Devolve o referencial dela, ou vazio se degenerada.
static func _placa(st: SurfaceTool, a: Dictionary) -> Dictionary:
	...
	var tipo: float = TIPOS[a["tipo"]]
	var cor := Color(1.0 if a["tipo"] == &"parabrisa" else 0.0, tipo, 0.0, 1.0)
	var cols := maxi(1, ceili(maxf(q[0].distance_to(q[1]),
		q[3].distance_to(q[2])) / PASSO))
	var linhas := maxi(1, ceili(maxf(q[0].distance_to(q[3]),
		q[1].distance_to(q[2])) / PASSO))
	for i in linhas:
		for j in cols:
			...
			for canto: Vector3 in [a0, a1, a2, a0, a2, a3]:
				...
				var m := Vector2(d.dot(u_eixo), d.dot(v_eixo)) - lo
				st.set_normal(-n)
				st.set_color(cor)
				st.set_uv(Vector2(m.x / tam.x, m.y / tam.y))
				st.set_uv2(m)
				st.add_vertex(canto)
	return {
		"tipo": a["tipo"],
