# -*- coding: utf-8 -*-
"""Pass 2 Fusca: silhueta lateral fiel a ref_01 + peso de sujeira + farol redondo.
NAO toca _vidros nem Marea.
"""
...
# Guard: keep vidros hash stable
vid_before = text[text.index("static func _vidros"): text.index("static func _frente_e_tras")]

# Slightly shorter/taller besouro for clearer side cupola
...
elif new_med not in text:
    raise SystemExit("MEDIDAS FUSCA line missing")

# Dirtier cream base tint (less clean)
...
static func _lataria_fusca(dados: Dictionary, comp: float, larg: float, capo: float,
	...
	# para o perfil lateral mostrar o "vale" entre os dois arcos (ref_01).
	...
	_face(dados, Vector2(0.10, comp * 0.18),
		Transform3D(Basis(Vector3.RIGHT, -0.72),
			Vector3(0.0, capo - 0.02, comp * 0.33)), Color(cor.r * 0.92, cor.g * 0.90, cor.b * 0.86), C_CAPO)
	...
	# Leitura lateral = dois humps sobre as rodas (ref_01).
	...
	for s: float in [1.0, -1.0]:
		...
		_caixa(dados, Vector3(0.34, altura_casco * 0.48, 0.38),
			...
			suja, suja, C_CAPO, false)
		# Ferrugem densa na base do arco (peso de sujeira das refs).
		...
		_caixa(dados, Vector3(0.36, altura_casco * 0.22, 0.42),
			...
			suja, suja, suja, false)
		# --- traseiro (ligeiramente maior — motor) ---
		...
		_caixa(dados, Vector3(0.38, altura_casco * 0.22, 0.46),
			Vector3(sx, ASSOALHO + altura_casco * 0.12, -comp * 0.30), ferrugem_escura,
			suja, suja, suja, false)
		# Soleira / running board enferrujada.
		...
		_face(dados, Vector2(comp * 0.28, 0.028),
			...
			chrome, C_PARACHOQUE)
		# Manchas de ferrugem na porta (baixo + meio).
	...
	for s: float in [1.0, -1.0]:
		...
		for z_arco: float in [comp * 0.30, -comp * 0.30]:
			...
			_face(dados, Vector2(0.30, 0.20),
				...
				cor, C_CAPO)
			# Chanfro dianteiro/traseiro do arco (quebra a caixa).
			var z_edge := z_arco + (0.22 if z_arco > 0.0 else -0.22)
			_face(dados, Vector2(0.26, altura_casco * 0.55),
				Transform3D(basis_lado * Basis(Vector3.UP, s * (-0.35 if z_arco > 0.0 else 0.35)),
					Vector3(sx2 - s * 0.02, ASSOALHO + altura_casco * 0.50, z_edge)),
				cor, suja)

	# Cabine alinhada ao _vidros (P0) — NAO alterar z0/recuo/lv.
...
static func _frente_e_tras_fusca(dados: Dictionary, luzes: Dictionary, comp: float,
	...
	# "Boca" baixa escura entre os para-lamas (ref_03).
	...
	for z: float in [zf, zt]:
		...
		# Para-choque em 3 segmentos (curva fraca PSX vs barra unica).
		...
		for s: float in [1.0, -1.0]:
			_face(dados, Vector2(larg * 0.22, 0.085),
				Transform3D(basis * Basis(Vector3.UP, s * 0.28),
					Vector3(s * larg * 0.34, ASSOALHO + 0.11,
						z + (0.04 if frente else -0.04))),
				Color(0.58, 0.58, 0.60), C_PARACHOQUE)
			# Overriders.
			_face(dados, Vector2(0.050, 0.22),
				Transform3D(basis, Vector3(s * larg * 0.18, ASSOALHO + 0.18,
					z + (0.014 if frente else -0.014))),
				Color(0.70, 0.70, 0.72), C_PARACHOQUE)
			# Ferrugem sob o para-choque / canto do para-lama.
			_face(dados, Vector2(0.16, 0.10),
				Transform3D(basis, Vector3(s * larg * 0.38, ASSOALHO + 0.06,
					z + (0.02 if frente else -0.02))),
	...
	# Farol "redondo" a 480p: caixa saliente + anel + losango + nucleo.
	var ox := larg * 0.46
	var yf := y + 0.06
	for s: float in [1.0, -1.0]:
		# Volume do farol no para-lama (nao flat na grade).
...
def replace_func(src: str, name: str, new_body: str) -> str:
    # Match from ## comment or static func through next ## or static at column 0 after this func
    # Find static func line
    pat = rf"(?:^##[^\n]*\n)*(?:^##[^\n]*\n)*static func {name}\(.*?\n)(?=^## |\Z|^static func )"
    # Simpler: find start index
    markers = []
    for m in re.finditer(rf"^static func {name}\(", src, flags=re.M):
        markers.append(m.start())
    if not markers:
        raise SystemExit(f"func {name} not found")
    start = markers[0]
    # Walk back over ## comment lines
    line_start = src.rfind("\n", 0, start) + 1
    while line_start > 0:
        prev = src.rfind("\n", 0, line_start - 1) + 1
        line = src[prev:line_start]
        if line.startswith("##"):
            start = prev
            line_start = prev
        else:
            break
    # Find next top-level static func or ## section that starts a new func after this one
    rest = src[start + 10:]
    m2 = re.search(r"\n(?=## [A-ZÁÉÍÓÚÂÊÔÃÕÀ]|static func )", rest)
    # Better: find next "\nstatic func " or "\n## " that precedes static
    m2 = re.search(r"\nstatic func ", src[start + 20:])
    if not m2:
        raise SystemExit(f"end of {name} not found")
    end = start + 20 + m2.start() + 1  # keep the newline before next static
    # But we may have ## comments belonging to next func — include them in cut by walking back
    block = src[start:end]
    # If block ends with ## comments for next func, trim
    lines = block.splitlines(keepends=True)
    while lines and lines[-1].startswith("##"):
        lines.pop()
    if lines and lines[-1].strip() == "":
        pass
    end = start + sum(len(x) for x in lines)
    return src[:start] + new_body + src[end:]

text2 = replace_func(text, "_lataria_fusca", new_lataria)
text2 = replace_func(text2, "_frente_e_tras_fusca", new_frente)

vid_after = text2[text2.index("static func _vidros"): text2.index("static func _frente_e_tras")]
if vid_after != vid_before:
    raise SystemExit("FORBIDDEN: _vidros changed")
if "static func _frente_e_tras_marea" not in text2:
    raise SystemExit("Marea frente missing")
if "_face_dois_lados(" in vid_after:
    raise SystemExit("_vidros gained dois_lados")
if "Color(0.72, 0.80, 0.86)" not in vid_after:
    raise SystemExit("vidros tint missing")

path.write_text(text2, encoding="utf-8")
print("pass2 mesh ok", path.stat().st_size)
print("medidas", [l for l in text2.splitlines() if "Modelo.FUSCA" in l and '"c"' in l][0])
print("tint", [l for l in text2.splitlines() if "TINTA_FUSCA" in l][0])
