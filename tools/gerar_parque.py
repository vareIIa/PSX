...
def nenufar(com_flor: bool) -> Image.Image:
    ...
    c, d = tela()
    for i in range(2 if com_flor else 3):
        r = D * rng.uniform(0.26, 0.34)
        x = D * rng.uniform(0.3, 0.7)
        y = D * rng.uniform(0.3, 0.7)
        ...
        d.polygon([(x, y),
                   ...
                   (x + math.cos(a + 0.3) * r * 1.1, y + math.sin(a + 0.3) * r * 1.1)],
                  fill=VAZIO)
        d.arc((x - r, y - r, x + r, y + r), 0, 360, fill=(46, 64, 38, 255), width=3)
    if com_flor:
        x, y = D * 0.5, D * 0.46
        corola(d, x, y, D * 0.24, 8, (238, 226, 234, 255), (232, 206, 120, 255))
    return c
