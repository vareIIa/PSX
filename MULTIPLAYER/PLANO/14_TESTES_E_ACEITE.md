...
| 4e | `./tools/mp_e2e.sh [--janela]` | **o jogo de verdade, de ponta a ponta**: duas cidades (headless) com `SondaE2E` — porta da casa da fumaça aberta/fechada/reaberta igual nas duas, item disputado numa mochila só, chat, save carregado pelo anfitrião no meio (recarga), saída do convidado de volta ao próprio mundo com a mochila, a profissão chega ao outro; carteira e conversa não; duas mãos na mesma prateleira (fusão); item de dentro da casa da rua pego da calçada; mapa do grupo; o item disputado pego pelo **[E] de verdade** (o raio da câmera tem de escolher o item); o convidado entra e sai de um cômodo com a validação em CORRIGIR e zero correção; zero SCRIPT ERROR da rede (43 conferências) | erro de outra frente vira aviso com o arquivo |
